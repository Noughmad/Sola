...
\begin{itemize}
 \item da sem magistrsko delo z naslovom \emph{Modeliranje "sirjenja svetlobe vzdol"z ograjenih teko"cekristalnih defektnih linij} izdelal samostojno pod mentorstvom prof. dr. Slobodana "Zumra in somentorstvom doc. dr. Miha Ravnika ter
...
Prepoznamo lastne na"cine valovanja v teko"cekristalnih vlaknih z razli"cnimi defekti vzdol"z osi in mo"zne polarizacije svetlobe, ki bi jih lahko dobili z vodenjem svetlobe skozi tak"sna vlakna. 
...
To je ve"cje od velikosti molekul, ki sestavljajo teko"ci kristal, zato lahko govorimo o redu dolgega dosega, je pa primerljivo z valovno dol"zino vidne svetlobe \cite{kleman}. 
...
Teko"ci kristali se obna"sajo kot efektivno elasti"cni medij, kjer direktorju lahko pripi"semo tri osnovne deformacijske na"cine: pahlja"co, zvoj in upogib.
...
Dvolomnost je lastnost snovi, v kateri je lomni koli"cnik odvisen od smeri polarizacije vpadne svetlobe \cite{landau-lifsic-optics}. 
Ve"cina dvolomnih snovi je enoosnih, kar pomeni, da obstaja ena izredna os z druga"cnim lomnim koli"cnikom, vse nanjo pravokotne smeri pa so enakovredni. 
Tipi"cne enoosne snovi so kristali s tetragonalno ali heksagonalno mre"zo, kjer je opti"cna os enaka po celem kristalu. 
...
Zaradi ob"cutljivosti na elektri"cno polje se teko"ci kristali uporabljajo v razli"cnih elektroopti"cnih napravah, na primer v nastavljivih svetlobnih filtrih \cite{filters-sneh}.
...
Poleg linearne in elipti"cne polarizacije lahko z njimi dose"zemo tudi osno simetri"cno polarizacijo, na primer radialno ali azimutalno \cite{polarization-converters-linear,polarization-converters-axial}. 

Obstaja ve"c metod za modeliranje "sirjenja svetlobe skozi opti"cno anizotropne snovi. 
Z Jonesovim vektorjem predstavimo elektri"cno polje svetlobe, ki se "siri v smeri $z$, kot vektor s komponentama v smereh $x$ in $y$, med katerima je lahko tudi fazni zamik. 
...
Jonesova in Muellerjeva metoda sta uporabni v sistemih z visoko simetrijo, manj pa v teko"cih kristalih, kjer je dvolomna os krajevno odvisna. 
Za modeliranje anizotropnih snovi je bolj primerna Berremanova metoda \cite{berreman,stallinga-berreman}, pri kateri je svetloba predstavljena s "stirimi transverzalnimi komponentami elektri"cnega in magnetnega polja. 
Berremanova metoda zataji v prisotnosti nezveznosti dielektri"cnega tenzorja, kot na primer v okolici teko"cekristalnih defektov \cite{metode-kriezis, hwang-rey}. 
...
V petem poglavju so opisani preizkusi metode, vklju"cno z "sirjenjem ravnega vala po praznem prostoru, po fotonskem kristalu in po teko"cekristalnem vlaknu. 
...
Polo"zaj gradnikov nima reda dolgega dosega v vseh smereh, zato se snov obna"sa kot teko"cina. 
...
Teko"cekristalne mezofaze se pojavijo v dolo"cenem temperaturnem in koncentracijskem razponu. 

Teko"ce kristale delimo na ve"c mezofaz glede na orientacijski in pozicijski red gradnikov. 
...
\begin{figure}[h]
\begin{center}
\subfigure{\begin{overpic}[height=100pt]{Nematic-Director} \put(86,46.5){\Large $\vec n$} \end{overpic}}
\subfigure{\includegraphics[height=100pt]{nematik-formuli}}
 \caption{Levo: Gradniki imajo razli"cne orientacije, povpre"cno orientacijo pa poda direktor $\vec n$. Desno: Strukturni formuli molekul 5CB (zgoraj) in 8CB (spodaj) \cite{wiki:lc}}
...
Namesto tega uporabimo povpre"cni kvadrat kota, ki je enakovreden kvadrupolnemu momentu \cite{kleman}
\begin{align}
...
V izrazu za $Q_{ij}$ nastopajo le produkti dveh komponent $n_i$, zato s tak"snim zapisom avtomatsko upo"stevamo simetrijo direktorja, saj zamenjava $\mathbf{n}$ z $-\mathbf{n}$ tenzorja $Q_{ij}$ ne spremeni. 
...
Na sliki \ref{fig:tsb} so prikazani trije na"cini deformacije direktorja, s katerimi lahko lokalno opi"semo poljubno krajevno odvisnost direktorja. 
Ti trije na"cini so pahlja"ca \angl{splay}, zvoj \angl{twist} in upogib \angl{bend}. 

Pri pahlja"cni deformaciji je neni"celna divergenca direktorja, pri zvoju z direktorjem vzporedna komponenta rotorja, pri upogibu pa pravokotna komponenta rotorja. 
Deformacijo zato kvantiziramo s temi tremi koli"cinami. 
Vsi trije na"cini deformacije so elasti"cni, zato je prispevek k prosti energiji sorazmeren s kvadratom deformacije. 
V teko"cih kristalih so v splo"snem elasti"cne konstante, ki pripadajo vsakemu izmed treh osnovnih na"cinov, med seboj razli"cne. 
Prispevek h gostoti proste energije zaradi elasti"cnih deformacij nematskega teko"cega kristala je tako enak
\begin{align}
...
Za numeri"cno modeliranje pa je bolj ugodno, "ce prosto energijo izrazimo s komponentami tenzorja $Q$, ki poleg direktorja vklju"cuje tudi stopnjo reda in dvoosnost \cite{copar-phd}. 
...
\end{align}
kjer je $T$ temperatura, $T^\ast$ najni"zja mo"zna temperatura podhlajene izotropne faze, $a$, $B$ in $C$ pa "cleni Landauovega razvoja, ki so odvisni od snovi. 
...
\end{align}
kjer je $\overline{\varepsilon}$ povpre"cna dielektri"cna konstanta, $\varepsilon_a^{\mathrm{mol}} = \varepsilon_{\parallel}^{\mathrm{mol}} - \varepsilon_{\perp}^{\mathrm{mol}}$ dielektri"cna anizotropija posamezne molekule, $E_i$ pa komponente zunanjega elektri"cnega polja. 
...
V snovi s pozitivno dielektri"cno anizotropijo je najugodnej"sa orientacija gradnikov vzporedna z zunanjim poljem. 
...
Ta vrednost je odvisna od jakosti elektri"cnega polja, vednar je preureditev teko"cega kristala tudi pri zelo mo"cnem polju ve"c velikostnih redov po"casnej"sa od nihanja opti"cnega polja s periodo velikostnega reda femtosekunde. 
"Ce na teko"ci kristal svetimo, ureditev molekul ne more slediti hitremu spreminjanju opti"cnega elektri"cnega polja, ampak nanje efektivno deluje povpre"cen kvadrat polja. 
...
V tem primeru se pojavijo obmo"cja z nedefiniranim direktorjem. 
...
V vlaknih nastopajo le linijski defekti, imenovani tudi disklinacije, zato se bomo posvetili zlasti tem. 
...
\end{align}
kjer kot $\theta$ opisuje smer direktorja, $\phi$ pa polo"zaj na zanki (svetlo moder krog na slikah \ref{fig:defekti-celi} in \ref{fig:defekti-polceli}). 

Direktorsko polje je zvezno povsod razen v jedru defekta, zato mora direktor pri $\phi = 0$ enak kot pri $\phi = 2\pi$, ko upo"stevamo se simetrijo $\vec n \leftrightarrow -\vec n$.
...
Mo"ci defekta pravimo tudi topolo"ska invarianta, saj se defekti znotraj teko"cega kristala lahko zdru"zujejo in pretvarjajo eden v drugega, vsota njihovih mo"ci pa se ohranja. 
...
V cilindri"cni geometriji s homeotropnimi robnimi pogoji ureditev z uniformnim direktorjem ni mogo"ca. 
...
Kljub temu, da robni pogoji vsiljujejo direktor v ravnini, pravokotni na os kapilare, ima lahko direktor znotraj kapilare tudi komponento, vzporedno z osjo. 
Pojavu imenujemo pobeg v tretjo dimenzijo, zaradi njega pa je lahko v kapilari direktorsko polje brez defektov. 
...
Do pobega v tretjo dimenzijo pride, "ce je razmerje $K_3/K_1$ manj"se od 13 \cite{kleman}, kar velja za ve"cino nematskih teko"cih kristalov. 
...
V smekti"cni fazi pahlja"cna deformacija nosi mnogo manj"so energijo kot ostala dva na"cina, saj je edina kompatibilna z ureditvijo v plasti. 
...
Z uporabo teko"cega kristala 8CB, ki tvori tako nematsko kot tudi smekti"cno fazo, je mogo"ce v laboratoriju sintetizirati vlakna z radialnim profilom direktorja \cite{peddireddy}. 
...
\begin{figure}[!ht]
 ...
 \caption{Rast vlaken z radialnim direktorjem na meji med 8CB in vodo \cite{peddireddy}}
 \label{fig:tvorjenje}
\end{figure}
...
V ena"cbah nastopata izvora elektromagnetnih polj, in sicer gostota prostih nabojev $\rho_f$ in gostota prostega toka $J_f$. 
...
Prostih nabojev tipi"cno v teko"cekristlaih sistemih ni, zato je $\rho_f=0$. 
...
Primer so temne pege v osi Laguerre-Gaussovih snopov, prikazanih na sliki \ref{fig:laguerre-gauss}. 
...
\begin{figure}[!ht]
 ...
 \caption{Intenziteta in polarizacija v nekaterih Laguerre-Gaussovih na"cinih. Slike (a), (b), (c) in (d) prikazujejo snope z ovojnimi "stevili $l = 0,1,2,3$. 
 Bela barva predstavlja obmo"cja z visoko intenziteto svetlobe v dveh ravninah, pravokotni in vzporedni s snopom. 
 Rde"ce pu"s"cice prikazujejo elektri"cno polje v ravnini grla snopa \cite{porenta}}.
...
"Ce pa je svetloba zelo "sibka, ne spremeni orientacije molekul in lahko privzamemo, da je direktorsko polje dolo"ceno z drugimi mehanizmi, tipi"cno s povr"sinskim pogoji.
...
Za izra"cun propagacije svetlobe skozi mo"cno opti"cno aniztropne teko"cekristalne vzorce smo implementirali metodo kon"cnih diferenc v "casovni domeni \angl{\acf{FDTD}} \cite{taflove}. 
...
V svoji metodi smo za odvod polja po vsaki koordinati v sredi"s"cu kocke uporabili povpre"cje odvodov na vseh "stirih robovih, ki potekajo v smeri izbrane koordinate, kot prikazuje slika \ref{fig:lattice-derivatives}. 
...
\centering
 ...
 Vrednost potrebujemo v sredi"s"cu kocke (modra pika), zato smo uporabili povpre"cje "stirih vrednosti na robu. 
 }
...
Pomembna lastnost obeh zgornjih ena"cb je, da med ra"cunanjem enega izmed polj potrebujemo le vrednosti drugega polja, ne pa tudi istega polja na sosednjih mestih. 
...
Tak material je nefizikalen, saj ima elektromagnetno polje v njem dodatne prostostne stopnje, polje pa ne sledi ve"c Maxwellovim ena"cbam. 

Komponenta $E_x$ elektri"cnega polja v obi"cajnem izotropnem mediju z dielektri"cnostjo $\eps$ in elektri"cno prevodnostjo $\sigma$ zado"s"ca Maxwellovi ena"cbi
\begin{align}
...
Oba prispevka lahko zmanj"samo, "ce pove"camo debelino robne plasti, s "cimer se pove"ca tudi "cas ra"cunanja. 
...
\begin{figure}[h]
 ...
 \thicklines
  \put(0,71){\line(1,6){11.7}}
  \put(0,71){\line(1,-6){11.8}}
 \end{picture}
 \hspace{-2.05mm}
...
\section{Lom valovanja}
Enostaven preizkus metode za modeliranje "sirjenja valovanja je prehod "cez mejo med medijema z razli"cnima lomnima koli"cnikoma. 
...
\end{align}
kjer je $\alpha$ vpadni kot valovanja, $\beta$ lomni kot oz. kot med pravokotnico in smerjo "sirjenja lomljenega vala, $n_1$ in $n_2$ pa sta lomna koli"cnika obeh medijev. 
...
\begin{align}
 \theta_B &= \arctan\left(\frac{n_2}{n_1}\right) = \arctan\sqrt{\frac{\varepsilon_2}{\varepsilon_1}}
\end{align}
Za prikaz delovanja smo uporabili vpadni kot, enak Brewsterjevemu. 
Na ta na"cin opazimo le vpadno in lomljeno valovanje, tako da lahko vedno preverimo, metoda izpolnjuje lomni zakon. 
Prikaz elektri"cnega polja ob prehodu meje, kjer je vpadni kot enak Brewsterjevemu, je na sliki \ref{fig:refraction-test}. 

\begin{figure}[h]
 \centering
 \begin{overpic}[width=.8\textwidth]{g_refraction_test}
  \put(3.4,36){\color{white} \large \bf I}
  \put(28,36){\color{white} \large \bf II}
  \put(70,36){\color{white} \large \bf III}
  \put(94,36){\color{white} \large \bf IV}
  \put(43,17.6){$\boldsymbol\alpha$}
  \put(58,22.5){$\boldsymbol\beta$}
 \end{overpic}
 \caption{Trenutna slika elektri"cnega polja ob prehodu valovanja v medij z druga"cnim lomnim koli"cnikom. 
 Barva predstavlja lokalno elektri"cno polje, "crna "crta lomno pravokotnico, bele pu"s"cice pa smer "sirjena valovanja. 
 Lomni koli"cnik na levi je enak $n_1 = 1$, na desni pa $n_2 = 1,\!594$. }
...
Pri Brewsterjevem kotu odbitega valovanja ni, zato so valovi ravni. 
...
Na desnem robu (IV) je absorbirajo"ca plast, kjer je vidna postopna absorpcija lomljenega valovanja. 
Absorpcija je tako dobra, da odbito valovanje na sliki ni vidno. 
Uporabljena metoda pravilno napove, da se svetloba s polarizacijo v ravnini "zarka in normale na povr"sino ob vpadu pod Brewsterjevim kotom ne odbije. 
...
\begin{align}
 \beta &= \arcsin \left(\frac{n_1}{n_2} \sin \alpha \right) = \arcsin \left( \frac{1}{1,\!594} \times \sin 58\degree \right) \approx 32\degree
\end{align}
kjer je $\beta$ lomni kot, $\alpha$ pa vpadni kot. 
Z opazovanjem valovanja na desnem delu slike \ref{fig:refraction-test} lahko potrdimo, da metoda pravilno opi"se lom svetlobe. 
...
Odli"cno kvantitativno ujemanje z analiticno pri"cakovano odvisnostjo je prikazano na sliki \ref{fig:test-uniform}.
To nadalje potrjuje pravilno delovanje metode v opti"cno anizotropni snovi. 

\begin{figure}[h]
\centering
\resizebox{.75\textwidth}{!}{\input{g_test_uniform}}
...
Za preverjanje metode smo modelirali enodimenzionalen fotonski kristal, kjer se izmenjujejo plasti z debelino $a/2$ in z izotropno dielektri"cnostjo $\varepsilon_1$ in $\varepsilon_2$, kot je prikazano na sliki \ref{fig:periodic-structure}. 
\begin{figure}[h]
 \centering
 \resizebox{.52\textwidth}{!}{\large \input{./Slike/periodic-structure.pdf_tex}}
...
\begin{figure}[h]
\centering
  \resizebox{.95\textwidth}{!}{\footnotesize \input{./Slike/bandgap.pdf_tex}}
...
Valovanje, ki se je od periodi"cne strukture zaradi prepovedanega pasu odbilo, smo zajeli v absorbirajo"ci plasti okrog celice. 

\begin{figure}[!htbp]
\centering
\resizebox{.75\textwidth}{!}{\input{g_test_periodic}}
...
To je posledica velikosti sistema, saj so teoreti"cni izra"cuni napravljeni za neskon"cen kristal, z metodo \acs{FDTD} pa lahko modeliramo le kon"cnega. 
...
\begin{figure}[!ht]
 \centering
  \begin{overpic}[width=.38\textwidth]{licp_0_68}\put(2,90){\color{white} \large \bf (a)}\end{overpic} \hspace{1mm}
  \begin{overpic}[width=.38\textwidth]{licp_0_78}\put(2,90){\color{white} \large \bf (b)}\end{overpic}
...
Redna komponenta, prikazana na sliki \ref{fig:pulse-0-mode}b, ob"cuti manj"si lomni koli"cnik in se zato "siri hitreje kot izredna. 
...
Zaradi dvolomnosti teko"cega kristala smo opazili razcep sunka na dve komponenti oz. dva stacionarna na"cina "sirjenja svetlobe. 
Obe komponenti sunka potujeta po vlaknu, polarizacija svetlobe znotraj vsakega na"cina pa ostaja nespremenjena. 
...
\begin{figure}[!htb]
 \centering
 \sunek{p1}
 \caption{Stacionarna na"cina "sirjenja svetlobe skozi vlakno z radialnim profilom direktorja ($s=+1$)}
...
Pri vodoravno polarizirani vpadni svetlobi je v enem izmed na"cinov ta ravnina navpi"cna, v drugem pa vodoravna. 
...
Kratek laserski sunek se podobno kot pri radialnem profilu razdeli na dva na"cina, rednega in izrednega, ki sta skladna s simetrijo direktorja. 
Oba na"cina sta prikazana na sliki \ref{fig:pulse-m1-mode}. 

\begin{figure}[!htbp]
 \centering
  \sunek{m1}
 \caption{Stacionarna na"cina "sirjenja svetlobe skozi vlakno s hiperboli"cnim profilom direktorja ($s=-1$)}
...
Spet sta vidni ravnini ni"celne intenzitete, kjer se smer elektri"cnega polja obrne, ki sta posledica polarizacije vpadne svetlobe. 
...
\begin{figure}[!htbp]
 ...
 \caption{Stacionarna na"cina "sirjenja svetlobe skozi vlakno z defektom z necelim ovojnim "stevilom $s=+1/2$}
...
\begin{itemize}
 \item Sunek linearno polarizirane svetlobe se vedno razdeli na dva stacionarna na"cina, ki ustrezata redni in izredni polarizaciji.
 \item Svetloba v obeh na"cinih je lokalno linearno polarizirana
 ...
 \item Stacionarna na"cina imata iste simetrijske ravnine, ki so presek simetrijskih ravnin teko"cega kristala in polarizacije vpadne svetlobe. 
\end{itemize}
...
Stacionarni na"cini "sirjenja valovanja, ki smo jih prepoznali v tem poglavju, torej niso odvisni le od ureditve teko"cega kristala, ampak tudi od polarizacije vpadne svetlobe. 
...
Pri enakomernem laserskem "zarku je svetloba v vlaknu z radialnim profilom direktorja kombinacija obeh lastnih na"cinov, prepoznanih v prej"snjem poglavju, ki se "sirita z razli"cnima hitrostma. 
Frekvenca svetlobe je nespremenjena in zato enaka po celotnem vlaknu, opazimo pa mo"cno krajevno odvisnost oblike polja. 
Za razliko od sunkovnega vzbujanja, kjer valovni paketi potujejo stran od izvora, pa se pri stalni svetlobi vzpostavi stacionarno stanje. 
Na vsakem mestu v vlaknu elektri"cno polje niha, njegova amplituda in polarizacija pa ostajata nespremenjeni. 
Strukture v elektri"cnem polju, prikazane na slikah v tem poglavju, se s "casom ne premikajo, zato jih lahko natan"cneje opazujemo. 

Trenutne slike elektri"cnega polja znotraj vlakna so prikazane na sliki \ref{fig:p1-cont-snaps}. 
Zaradi jasnosti smo uporabili vlakno z ni"zjo dvolomnostjo kot v sunkovnem na"cinu; tu je bila razlika med izrednim in rednim lomnim koli"cnikom le $\Delta n = 0,\!04$. 
Zna"cilno dol"zino pojavov v dvolomnem vlaknu lahko ocenimo kot
\begin{align}
 d_1 \sim \frac{\lambda}{\Delta n} 
\end{align}
kjer je $\lambda$ valovna dol"zina svetlobe, $\Delta n$ pa razlika lomnih koli"cnikov. 
Izbrali smo tak"sno dvolomnost, da je bila ta dol"zina mnogo ve"cja od valovne dol"zine, zato smo lahko pojave natan"cneje opazovali. 
Vse slike v tem poglavju prikazujejo intenziteto svetlobe in trenutno elektri"cno polje na razli"cnih polo"zajih v vlaknu. 
V vlakno vsaki"c vstopa svetloba z linearno polarizacijo v vodoravni smeri. 
Prva slika (a) prikazuje polje eno valovno dol"zino po vstopu v vlakno, kjer je ravno "ze vidno odstopanje od linearnega profila. 
Na tretji sliki (c) je vsaki"c polje, kjer je fazni zamik med lastnima na"cinoma z redno in izredno polarizacijo enak polovi"ci nihaja. 
Pri izbranih parametrih se to zgodi pribli"zno 15 $\mu$m po vstopu v vlakno. 
Dodatno smo prikazali vmesno stanje (c), ki nastopi 10 $\mu$m po vstopu v vlakno, kjer je zamik med obema lastnima na"cinoma tretjina nihaja. 
...
Konfiguracija pribli"zno ustreza defektu z ovojnim "stevilom $+1$, presek polja pa je podoben stacionarnemu na"cinu vlakna, prikazanemu na desni strani slike \ref{fig:pulse-p1-mode}. 
...
Posnetki elektri"cnega polja v vlaknu z defektom mo"ci $s=+1/2$ so prikazani na sliki \ref{fig:p12-cont-snaps}. 
...
Za razlike od defekta mo"ci $+1$, ki ga opazimo v vlaknu z radialnim profilom, pa je na zgornji sliki jasno vidna tudi komponenta svetlobe s polarizacijo, ki je pravokotna na vpadno polarizacijo. 
Direktor tam oklepa kot 45\degree~s polaraziracijo vpadne svetlobe, zato ureditev teko"cega kristala lokalno deluje kot plo"s"cica $\lambda/2$, ki polarizacijo svetlobe zavrti za 90\degree. 
...
V tem primeru opazimo izmenjevanje uniformnega elektri"cnega polja in polja z defektom mo"ci $-1$, pojavi pa se tudi vmesno stanje z defektom mo"ci $-1/2$. 
Na sliki \ref{fig:p12-cont-snaps}c je lepo viden hiperboli"cen profil polarizacije svetlobe. 
...
Tu je pomembno, da pri stacionarnih na"cinih ni vsa svetloba polarizirana vzporedno z vpadno polarizacijo.
...
Pri konstantnem svetenju pa se lastni na"cini se"stejejo tako, da imajo obmo"cja, kjer topologija defekta narekuje polarizacijo v smeri, pravokotni na vpadno polarizacijo, enako intenziteto svetlobe kot okolica. 
...
"Ce "zelimo, da se stacionarna na"cina propagacije svetlobe po vlaknu z defektom mo"ci $1/2$ (na sliki \ref{fig:pulse-p12-mode}) se"stejeta v radialno polarizacijo, mora biti fazna razlika med njima pol nihaja. 
...
\chapter{Zaklju"cek}
V nalogi smo preu"cili "sirjenje kratkih sunkov in stalne laserske svetlobe po te\-ko\-"ce\-kris\-tal\-nih vlaknih z defektnimi linijami vzdol"z osi. 
...
Laserski sunek, ki ima na za"cetku linearno polarizacijo, razpade na dva stacionarna na"cina "sirjenja, znotraj katerih polarizacija svetloba tvori defekt, ki je po ovojnem "stevilu enak defektu v teko"cem kristalu. 
...
Vsaj nekatere rezultate v tej nalogi bo mo"zno eksperimentalno preveriti, npr. s po"silanjem svetlobe skozi teko"cekristalna vlakna z radialnim direktorskim profilom, ki so jih nedavno ustvarili v laboratorijih. 
...
Pravilno modeliranje in prepoznavanje zapletenih struktur olaj"sa povezovanje med teoreti"cnimi in eksperimentalnimi raziskavami. 
Ob dodatku opti"cno aktivnih barvil lahko raziskujemo zanimive pojave, ki so jih opazili v laboratorijih, npr.~lasersko svetenje teko"cekristalnih kapljic in vlaken. 
Nadalje bi lahko preu"cevali materiale z zapleteno strukturo, kot so samourejeni metamateriali in fotonski kristali. 
Raziskovali bi lahko tudi in prehod svetlobe med sklopljenimi resonatorji, valovodi in drugimi opti"cnimi elementi. 
