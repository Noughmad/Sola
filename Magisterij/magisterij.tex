...
Njihova najpomembnej"sa opti"cna lastnost je dvolomnost, zaradi "cesar so osrednji opti"cno aktivni materiali v sodobni optiki in fototniki\cite{optics-lcd}.
...
Na prosto energijo in s tem na ureditev teko"cega kristala v zunanjih elekricnih ali opticnih poljih klju"cno vplivajo predvsem krajevno spreminjanje direktorja, temperatura in dielektri"cna interakcija med molekulami in zunanjim poljem. 
...
\begin{align}
 f_{\mathrm{el}} &= \frac{1}{2}L_1\qodv{ij}{k}\qodv{ij}{k} + \frac{1}{2}L_2 \qodv{ij}{j}\qodv{ik}{k} + \frac{L_3}{2}Q_{ij}\qodv{kl}{i}\qodv{kl}{l}\;,
\end{align}
kjer so $L_i$ ela"sti"cne konstante, enake $L_i = $
...
\begin{align}
 f_{\mathrm{el}} &= \frac{1}{2}L \qodv{ij}{k}\qodv{ij}{k}
\end{align}
kjer je $L$ edina elasti"cna konstanta\cite{ravnik-zumer-ldg}.
...
Tak"snim obmo"cjem, kjer orientacijski red ne dr"zi, pravimo defekti ali disklinacije. 
...
\begin{figure}[h]
\centering
\subfigure{\includegraphics[height=150px]{director-profile-cylinder-kleman}}
\subfigure{\includegraphics[clip, trim=0cm -3mm 0cm 0cm]{escaped-director-energy}}
\caption{Levo: Prerezi valja s singularno (a) in nesingularno pobeglo defektno linijo (b). 
...
V pobeglem profilu pa sta prisotni tako pahlja"cna kot upogibna deformacija. Energija tega stanja je odvisna od konstant $K_1$ in $K_3$. Do pobega pride, "ce je razmerje $K_3/K_1$ manj"se od 13, kar velja za ve"cino nematskih teko"cih kristalov. Z minimizacijo energije lahko izpeljemo ravnovesni radialni profil direktorja v cilindri"cnih koordinatah. V poenostavljenem primeru, ko sta elasti"cni konstanti $K_3$ in $K_1$ enaki, je energijsko najugodneje"se stanje\cite{kleman} 
\begin{align}
 \vec n &= (n_r, n_\phi, n_z) = (\cos \chi(r), 0, \sin \chi(r)) \\
 \chi(r) &= 2 \arctan \frac{R-r}{R+r}
\end{align}
kjer je $R$ polmer valja. Tak"sen profil je prikazan na sliki \ref{fig:director-profiles}b. V primeru pobega v tretjo dimenzijo je direktor povsod dobro definiran. "Ce pa do pobega ne pride, je tik ob osi valja defekt. Okrog defekta je direktorsko polje radialno, torej je to linijski defekt z ovojnim "stevilom $s=+1$. 
...
\begin{figure}[h]
 \centering
 \subfigure{\includegraphics[height=150px]{tvorjenje}}
 \subfigure{\includegraphics[height=150px]{tvorjenje2}}
...
Ta je brezsleden, zato tudi dielektri"cni tenzor zapi"semo kot vsoto izotropnega in brezslednega dela\cite{degennes, ravnik-zumer-ldg}
...
Podobno bi lahko poleg elektri"cne prevodnosti $\sigma$ upo"stevali magnetne izgube $\sigma^*$. 
...
\begin{align}
 ...
 (\nabla\times \vec A)_x|_{i,j,k}^n &= \frac{1}{4\Delta y}\sum_{\sumij{i}{k}} A_z|_{i',j+1,k'}^{n} - A_z|_{i',j,k'}^{n} \\
 (\nabla\times \vec A)_y|_{i,j,k}^n &= \frac{1}{4\Delta z}\sum_{\sumij{j}{i}} A_x|_{i',j',k+1}^{n} - A_x|_{i',j',k}^{n} \\
 (\nabla\times \vec A)_z|_{i,j,k}^n &= \frac{1}{4\Delta x}\sum_{\sumij{k}{j}} A_y|_{i+1,j',k'}^{n} - A_y|_{i,j',k'}^{n}
\end{align}
...
Za prisotnost fotonskega prepovedanega pasu potrebujemo kristal oz. periodi"cno strukturo, kjer je perioda primerljiva z valovno dol"zino svetlobe. 
...
Ra"cunsko metodo smo preizkusil na sistemu, ki je podoben obravnavanim, "se vedno pa lahko napovemo rezultat. 
V ta namen smo modeliral cilindri"cno vlakno, v katerem je uniformen dvolomen kristal, vanj pa po"sljemo kratek laserski sunkek. 
...
Teko"cekristalna vlakna, pridobljena v laboratoriju, imajo radialen direktorski profile\cite{peddireddy}. 
Ugotavljali, kako defekt v sredici vlakna vpliva na "sirjenje svetlobe. 
...
"Ce na polarizacijo gledamo kot na vektor z direktorsko simetrijo, lahko z ena"cbo (\ref{eq:winding-number}) obema na"cinoma priredimo ovojno "stevilo $+1$. 
Dodatno pa imata oba na"cina "se defektno ravnino, kjer se polarizacija svetlobe obrne, intenziteta svetlobe pa je enaka 0. 
...
Poljuben direktorski profil je mogo"ce teko"cemu kristalu vsiliti z zunanjim poljem, nato pa ga stabilizirati s polimerizacijo. 
...
Kak"sna izmed teh dve zrcalnih osi je prisotne tudi v direktorskem defektu, ne pa nujno obe. 
...
V zgorjih opa"zanjih lahko najdemo podobnosti in izpeljemo splo"sne zakonitosti. 
\begin{itemize}
...
Mnogo bolj enostavno pa je opazovanje z laserjem, ki sveti konstantno. 

\section{Radialni profil direktorja}

Pri enakomernem laserskem "zarku je svetloba v vlaknu kombinacija obeh lastnih na"cinov, ki se "sirita z razli"cnima hitrostma. 
