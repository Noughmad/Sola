...
fit[0:80] h0(x) "data/g_konvergenca_ho_0.1.csv" u 1:2 via a,A
fit[0:170] h1(x) "data/g_konvergenca_ho_1.csv" u 1:2 via b,B
fit[0:170] l0(x) "data/g_konvergenca_L_0.1.csv" u 1:2 via c,C
fit[0:170] l1(x) "data/g_konvergenca_L_1.csv" u 1:2 via d,D

plot[0:300][0:20] "data/g_konvergenca_ho_0.1.csv" u 1:2 t "HO, $\\lambda = 0.1$", h0(x) t sprintf("$r = %.2g$", a), \
"data/g_konvergenca_ho_1.csv" u 1:2 t "HO, $\\lambda = 1$", h1(x) t sprintf("$r = %.2g$", b), \
"data/g_konvergenca_L_0.1.csv" u 1:2 t "Lanczos, $\\lambda = 0.1$", l0(x) t sprintf("$r = %.2g$", c), \
"data/g_konvergenca_L_1.csv" u 1:2 t "Lanczos, $\\lambda = 1$", l1(x) t sprintf("$r = %.2g$", d)
